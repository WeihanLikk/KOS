...
static unsigned int size_kmem_cache[ PAGE_SHIFT ] = { 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048 };
...
extern void slab_info()
{
	...
	tmp_cache = cache_chain;
	kernel_printf( "Slab System :\n" );
	while ( tmp_cache )
	{
		kernel_printf( "\t%d: size-%x color-%x", num, tmp_cache->obj_size, tmp_cache->colour );
		num++;
		tmp_cache = tmp_cache->next;
		if ( num % 3 == 0 )
			kernel_printf( "\n" );
	}
	kernel_printf( "\tTotal cache number: %x \n", num );
}

int kmem_cache_number()
{
	int num = 0;
	struct kmem_cache *tmp_cache;
	tmp_cache = cache_chain;
...
//color can be 0,1,2
static int cache_set_colornum( struct kmem_cache *cache, unsigned int color )
{
	cache->colour = color;
	return 1;
#ifdef SLAB_DEBUG
	...
	int allign = cache->colour_off / color;
	if ( !allign && color != 1 )
	{
#ifdef SLAB_DEBUG
		kernel_printf( "color num is too big: %x\n", colornum );
...
void init_kcache( struct kmem_cache *cache, unsigned int objsize, unsigned int colour )
{
	cache->obj_size = objsize;
	cache->colour = colour;
	...
	//kernel_printf("f9\n");
	int k = cache_set_colornum( cache, colour );
	if ( !k )
	{
...
void init_slab()
{
	...
	init_kcache( &( kmalloc_caches[ 0 ] ), size_kmem_cache[ 0 ], 0 );
	for ( i = 1; i < PAGE_SHIFT; i++ )
	{
		init_kcache( &( kmalloc_caches[ i ] ), size_kmem_cache[ i ], 0 );
#ifdef SLAB_DEBUG
		kernel_printf( "%x", i );
#endif  // ! SLAB_DEBUG
	...
	kernel_printf( "sucessfully load static cache into cache chain!\n" );
#ifdef SLAB_DEBUG
...
struct kmem_cache *get_cache_with_create( unsigned int size, unsigned int color )
{
	...
	{
		//kernel_printf("f3\n");
		if ( tmp_cache->obj_size == size && tmp_cache->colour == color )
		{
			return tmp_cache;
		}
		tmp_cache = tmp_cache->next;
...
struct kmem_cache *kmem_cache_create( const char *name, unsigned int objsize, unsigned int colour )
{
	...
	//kernel_printf("f7\n");
	init_kcache( new_cache, objsize, colour );
	kernel_printf( "dynamically creating new cache success!\n" );
	struct kmem_cache *tmp_cache = cache_chain;
	while ( tmp_cache->next )
	{
		tmp_cache = tmp_cache->next;
	}
	tmp_cache->next = new_cache;
	//kernel_printf("f8\n");
	return new_cache;
}
...
static void page_off_cache( struct page *page, struct kmem_cache *cache )
{
	...
	page->slabp = 0;	// Point to the free-object list in get_cache_with_create page
}

void obj_cache_delete( unsigned int size, unsigned int color )
{
	struct kmem_cache *tmp_cache, *d_cache;
	tmp_cache = cache_chain;
	while ( tmp_cache )
	{
		if ( tmp_cache->next->obj_size == size && tmp_cache->next->colour == color )
		{
			d_cache = tmp_cache->next;
			tmp_cache->next = tmp_cache->next->next;
			kmem_cache_delete( d_cache );
			break;
		}
		tmp_cache = tmp_cache->next;
	}
