...
{
    if (!node){  //this node has not mounted any vma(normallly impossible)
        #ifdef VM_DEBUG
    ...
    if (!vma){  //this node has not mounted any vma(normallly impossible)
        #ifdef VM_DEBUG
            kernel_printf("In rb_search:there is a node not mounted any vma.\n", mm);
        #endif //VM_DEBUG
        return 0;
    }
    if(vma->vm_end > addr && vma->vm_start <= addr){ //vma is found.
        #ifdef VM_DEBUG
...
static void rbtree_left_rotate(struct rb_root *root, Node *left)
{
    // 设置left的右孩子为right
...
static void rbtree_right_rotate(struct rb_root *root, Node *y)
{
    // 设置x是当前节点的左孩子。
    Node *x = y->left;
...
int insert_rbtree(struct rb_root *root, Node * node){
	Node *y = NULL;
    Node *x = root->node;
