...
u32 delete_dentry( struct dentry *this )
{
	struct dentry *parent = this->d_parent;
	dput( parent );
	iput( this->d_inode );
	if ( this->d_inode->i_count == 0 )
	{
		u32 err = this->d_inode->i_sb->s_op->delete_inode( this );
		if ( err )
			return err;
	}
	dcache->c_op->d_delete( dcache, this );
