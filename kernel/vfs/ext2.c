...
struct file_operations ext2_file_operations = {
	.read = generic_file_read_write,
	.write = generic_file_read_write,
...
u32 ext2_create( struct inode *inode, struct dentry *dentry, u32 mode, struct nameidata *nd )
{
	return 0;
}
...
struct inode *ext2_create_inode( struct dentry *dentry, struct qstr *name )
{
	struct inode *inode = dentry->d_inode;
	kernel_printf( "%p", inode );
	struct inode *new_inode = (struct inode *)kmalloc( sizeof( struct inode ) );
	u32 group_base = ext2_group_base_sect( inode );
	u32 sectors_per_block = inode->i_blksize >> SECTOR_SHIFT;
	u32 inode_bitmap_base = group_base + sectors_per_block;
	u32 inode_table_base = group_base + ( sectors_per_block << 1 );
	// 清除块位图上所有相关的位
	struct ext2_base_information *ext2_BI = (struct ext2_base_information *)inode->i_sb->s_fs_info;
	u32 inodes_per_group = ext2_BI->sb.attr->inodes_per_group;
	u32 blocks_per_group = ext2_BI->sb.attr->blocks_per_group;
	u32 groupNo = ( inode->i_ino - 1 ) / inodes_per_group;

	struct inode *dummy = (struct inode *)kmalloc( sizeof( struct inode ) );
	dummy->i_blksize = inode->i_blksize;
	dummy->i_sb = inode->i_sb;
	dummy->i_ino = groupNo * inodes_per_group + 1;
	u32 block_bitmap_base = ext2_group_base_sect( dummy );

	u32 i, j;
	u32 err;
	u32 curBit = 1;
	u32 maxSect = inodes_per_group / BITS_PER_BYTE / SECTOR_SIZE;
	u8 buffer[ SECTOR_SIZE ];
	for ( i = 0; i < maxSect; i++ )
	{
		err = read_block( buffer, inode_bitmap_base + i, 1 );
		if ( err )
		{
			// return -EIO;
			kernel_printf( "%d", err );
		...
		for ( j = 0; j < SECTOR_SIZE * BITS_PER_BYTE; j++ )
		{
			curBit = get_bit( buffer, j );
			// kernel_printf( "%d", curBit );
			if ( !curBit )
				break;
		}
	...
	if ( i >= maxSect )
	{
		kernel_printf( "Full inode_bitmap" );
		// return -ENFILE;
		kernel_printf( "ENFILE" );
		return NULL;
	}
	// kernel_printf( "i_size = %d", inode->i_size);
	set_bit( buffer, j );
	write_block( buffer, inode_bitmap_base + i, 1 );

	u32 ino = groupNo * inodes_per_group + i * SECTOR_SIZE + j;

	struct address_space *mapping = &( inode->i_data );
	struct vfs_page *curPage;
	struct condition cond;

	u32 curPageNo = mapping->a_op->bmap( inode, ( ( inode->i_size - 1 ) / inode->i_blksize ) );
	if ( !curPageNo )
	{
		// kernel_printf( "i_blocks = %d", inode->i_blocks );
		// return -ENOENT;
		kernel_printf( "-ENOENT curpage", inode->i_size );
		return NULL;
	}
	...
	if ( !curPage )
		curPage = page_to_cache( curPageNo, mapping );

	u8 *data = curPage->p_data;

	// for test
	// struct ext2_dir_entry *temp = (struct ext2_dir_entry *)data;
	// kernel_printf( "file_type= %d\n,ino = %d,name = %s,name_len = %d, rec_len = %d", temp->file_type, temp->ino, temp->name, temp->name_len, temp->rec_len );
	// return NULL;
	// u8 cur = inode->i_size % inode->i_blksize;
	// struct ext2_dir_entry *temp = (struct ext2_dir_entry *)( curPage->p_data );
	// kernel_printf( "%d\n", inode->i_size / inode->i_blksize - 1 );
	// kernel_printf( "i_size = %d\n", inode->i_size );
	// for ( int i = 0; i < inode->i_blksize / 4; i += sizeof( struct ext2_dir_entry ) )
	// {
	// 	temp = (struct ext2_dir_entry *)( curPage->p_data + i );
	// 	kernel_printf( "file_type= %d\n,ino = %d,name = %s,name_len = %d, rec_len = %d\n", temp->file_type, temp->ino, temp->name, temp->name_len, temp->rec_len );
	// }
	// return NULL;
	u8 *cur = data;
	struct ext2_dir_entry *ex_dir_entry;
	while ( *cur != 0 && cur != data + inode->i_blksize )
	{
		ex_dir_entry = (struct ext2_dir_entry *)cur;
		kernel_printf( "file_type= %d\n,ino = %d,name = %s,name_len = %d, rec_len = %d\n",
					   ex_dir_entry->file_type, ex_dir_entry->ino, ex_dir_entry->name, ex_dir_entry->name_len, ex_dir_entry->rec_len );
		if ( cur + ex_dir_entry->rec_len < data + inode->i_blksize )
			cur += ( ex_dir_entry->rec_len );
		else
			break;
	}

	struct ext2_dir_entry *new_dir_entry = (struct ext2_dir_entry *)kmalloc( sizeof( struct ext2_dir_entry ) );
	new_dir_entry->file_type = EXT2_FT_REG_FILE;
	new_dir_entry->ino = ino;
	kernel_strcpy( new_dir_entry->name, name->name );
	new_dir_entry->name_len = name->len;
	u16 last_rec_len = 8 + ex_dir_entry->name_len / 4 * 4 + 4;
	new_dir_entry->rec_len = ex_dir_entry->rec_len - last_rec_len;
	ex_dir_entry->rec_len = last_rec_len;

	kernel_memcpy( (void *)cur, (void *)ex_dir_entry, last_rec_len );
	kernel_memcpy( (void *)( cur + last_rec_len ), (void *)new_dir_entry, 8 + new_dir_entry->name_len / 4 * 4 + 4 );
	// kernel_printf( "%d", (u32)*cur );
	cur = data;
	while ( *cur != 0 && cur != data + inode->i_blksize )
	{
		ex_dir_entry = (struct ext2_dir_entry *)cur;
	...
	ext2_writepage( curPage );
	// struct ext2_dir_entry *temp = (struct ext2_dir_entry *)( curPage->p_data );
	// for ( int i = 0; i <= cur; i += sizeof( struct ext2_dir_entry ) )
	// {
	// 	temp = (struct ext2_dir_entry *)( curPage->p_data + i );
	// 	kernel_printf( "file_type= %d\n,ino = %d,name = %s,name_len = %d, rec_len = %d", temp->file_type, temp->ino, temp->name, temp->name_len, temp->rec_len );
	// }
	// return NULL;
	// inode->i_size += sizeof( struct ext2_dir_entry );
	...
	for ( i = 0; i < maxSect; i++ )
	{
		err = read_block( buffer, block_bitmap_base + i, 1 );
		if ( err )
		{
			// return -EIO;
			kernel_printf( "EIO" );
	...
	if ( i >= maxSect )
	{
		kernel_printf( "Full block_bitmap" );
		// return -ENFILE;
		kernel_printf( "-ENFILE" );
		return NULL;
	}
	set_bit( buffer, j );
	write_block( buffer, inode_bitmap_base + i, 1 );
	u32 blockno = block_bitmap_base + ( i * SECTOR_SIZE + j ) * sectors_per_block * SECTOR_SIZE;
	blockno = blockno - ( (struct ext2_base_information *)( inode->i_sb->s_fs_info ) )->ex_base;
	blockno = blockno / ( sectors_per_block * SECTOR_SIZE );
	// TODO: new_ext2_inode
	u32 inode_sect = group_base + 2 * ( inode->i_blksize >> SECTOR_SHIFT ) +
					 ( ( new_inode->i_ino - 1 ) % inodes_per_group ) / ( SECTOR_SIZE / ext2_BI->sb.attr->inode_size );
	//u32 ex_inode = (struct ext2_inode *)( buffer + ( new_inode->i_ino - 1 ) % ( SECTOR_SIZE / ext2_BI->sb.attr->inode_size ) * ext2_BI->sb.attr->inode_size );

	struct ext2_inode *ext2_inode = (struct ext2_inode *)kmalloc( sizeof( struct ext2_inode ) );
	ext2_inode->i_size = 0;
	ext2_inode->i_links_count = 1;
	ext2_inode->i_blocks = 1;
	ext2_inode->i_block[ 0 ] = blockno;
	for ( i = 0; i < EXT2_N_BLOCKS; i++ )
		ext2_inode->i_block[ i ] = 0;

	err = read_block( buffer, inode_sect, 1 );
	kernel_memcpy( (void *)buffer, (void *)ext2_inode, sizeof( ext2_inode ) );
	err = write_block( buffer, inode_sect, 1 );

	// TODO: new_inode
	new_inode->i_count = 1;
	new_inode->i_blksize = inode->i_blksize;
	new_inode->i_sb = inode->i_sb;
	new_inode->i_ino = ino;
	new_inode->i_op = ( new_dir_entry->file_type == EXT2_FT_DIR ) ? ext2_inode_operations : ext2_inode_operations + 1;
	new_inode->i_fop = &ext2_file_operations;
	INIT_LIST_HEAD( &new_inode->i_dentry );
	new_inode->i_data.a_page = (u32 *)kmalloc( EXT2_N_BLOCKS * sizeof( u32 ) );
	if ( !new_inode->i_data.a_page )
	{
		// return -ENOMEM;
		kernel_printf( "ENOMEM" );
		return NULL;
	}
	for ( i = 0; i < EXT2_N_BLOCKS; i++ )
		new_inode->i_data.a_page[ i ] = ext2_inode->i_block[ i ];
	new_inode->i_data.a_host = new_inode;
	new_inode->i_data.a_pagesize = new_inode->i_blksize;
	new_inode->i_data.a_op = &ext2_address_space_operations;
	INIT_LIST_HEAD( &new_inode->i_data.a_cache );

	kernel_printf( "Stop here" );
	// ((struct ext2_base_information*) inode->i_sb->s_fs_info)->
	return new_inode;
}
