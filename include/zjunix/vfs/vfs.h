...
struct file_operations
{
	...
	u32 ( *read )( struct file *, u8 *, u32, u32 *, u32 );
	// 从文件的*offset处开始写入count个字节，然后增加*offset的值（一般与文件指针对应）
	u32 ( *write )( struct file *, u8 *, u32, u32 *, u32 );
...
u32 delete_dentry( struct dentry * );
...
u32 generic_file_read_write( struct file *, u8 *, u32, u32 *, u32 );
u32 generic_file_flush( struct file * );
struct vfs_page *page_to_cache( u32, struct address_space * );

// mount.c
u32 mount_ext2();
u32 follow_mount( struct vfsmount **, struct dentry ** );
struct vfsmount *lookup_mnt( struct vfsmount *, struct dentry * );
u32 mount_fs( u8 *, struct dentry * );
